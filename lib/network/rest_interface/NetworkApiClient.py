...
class NetworkApiClient:
	...
	@staticmethod
	def __process_stream_response(response: Response, request: Request) -> typing.Iterator:
		for chunk in response.iter_lines():
			yield request.deserialize_object(json.loads(chunk))
	...
	def execute(self, request: Request, headers: Optional[Dict] = None):
		...
		if request.stream:
			return self.__process_stream_response(response, request)
